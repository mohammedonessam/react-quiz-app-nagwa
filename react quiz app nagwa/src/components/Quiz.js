...
const Quiz = ({ showQuiz, question, quizs, checkAnswer, correctAnswer, selectedAnswer, questionIndex, nextQuestion, showTheResult,playerName }) => {
            let answers={
                "options": [
                    "adverb",
                    "verb",
                    ...
                    <div className="col-lg-8">
                        <div className="card p-4" style={{ background: '#3d3d3d', borderColor: '#646464' }}>
                            <div className="d-flex justify-content-between gap-md-3">
                                <h5 className='mb-2 fs-normal lh-base'>What kind of word is this? =={'>'} {question?.question}</h5>
                                <h5 style={{ color: '#60d600', width: '100px', textAlign: 'right' }}>{Math.floor(quizs.indexOf(question)*100 /quizs?.length) } %</h5>
                            </div>
                            <div>
                                {
                                    answers?.options?.map((item, index) => <button
                                        key={index}
